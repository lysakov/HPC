...
struct Domain
{
    double x1, x2;
    double y1, y2;
    Domain(double x1 = 0.0, double x2 = 0.0, double y1 = 0.0, double y2 = 0.0): x1(x1), x2(x2), y1(y1), y2(y2) {}
};
...
public:
        ...
        M(M), N(N), domain(domain)
    {
        ...
        for (int i = 0; i < M + 1; ++i) {
            for (int j = 0; j < N + 1; ++j) {
                ...
                if (i == 1) {
                    B[i*(N + 1) + j] += k(x(i) - 0.5*h1, y(j))*phi_1(y(j))/(h1*h1);
                }
                if (i == M - 1) {
                    B[i*(N + 1) + j] += k(x(i) + 0.5*h1, y(j))*phi_2(y(j))/(h1*h1);
                }
                if (j == 1) {
                    B[i*(N + 1) + j] += k(x(i), y(j) - 0.5*h2)*phi_3(x(i))/(h2*h2);
                }
                if (j == N - 1) {
                    B[i*(N + 1) + j] += k(x(i), y(j) + 0.5*h2)*phi_4(x(i))/(h2*h2);
                }
            }
        }
        #undef x
    ...
    void finalize()
    {
        ...
        for (int i = 0; i < M + 1; ++i) {
            for (int j = 0; j < N + 1; ++j) {
                if (i == 0) {
                    w[i*(N + 1) + j] = phi_1(domain.y1 + j*h2);
                    continue;
                }
                if (i == M) {
                    w[i*(N + 1) + j] = phi_2(domain.y1 + j*h2);
                    continue;
                }
                if (j == 0) {
                    w[i*(N + 1) + j] = phi_3(domain.x1 + i*h1);
                    continue;
                }
                if (j == N) {
                    w[i*(N + 1) + j] = phi_4(domain.x1 + i*h1);
                    continue;
                }
            }
        }
